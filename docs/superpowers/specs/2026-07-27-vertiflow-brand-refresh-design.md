...
| **Measurement** | One field on **PKBA's existing inscription form** — *"Comment tu nous as connus ?"*. No new base, no VertiFlow-held personal data, no engineering. |
| Leading indicator | `/commencer` views and click-throughs to PKBA inscription |
...
**Brand-first, shop inside it.** Decided 2026-08-05. The site is the brand's home — story,
club, spots, journal — and the shop is a strong section within it, not the front door. A
store federates nobody, and the 2027 national play needs a brand people can visit.

The structural move: **the collection is content, the permanent range is commerce.** They
stop being the same thing.

| Route | What it is |
|---|---|
| `/` | Not a product grid. Statement, the door, the current drop, the story strip, journal teasers. |
| `/collection/[slug]` | **The drop as an editorial page** — photography, the idea, the pieces in context. What makes a drop an event rather than four new rows. |
| `/boutique` + `/boutique/[slug]` | *L'essentiel*, the permanent nine. Straight commerce, fast and clean. |
| `/commencer` | The door. Six questions, links to PKBA's inscription. No form. |
| `/a-propos` | The story, including the injury and the comeback. |
| `/journal` + `/journal/[slug]` | MDX. Door articles and the commercial SEO set. |
| `/panier`, `/checkout`, `/success`, `/cancel` | Ported from the working pipeline, logic untouched. |
| 6 legal routes | One shared layout, content ported as-is. |

Replaced by the above: 9 hand-written product pages, `product-detail.html`,
`details-produit.html`, `blog.html` and its 3 article pages, `about.html`. Deleted
outright: `sign-in.html`, `sign-up.html`, `no-color.html`.

~25 files become ~16 routes, three of them dynamic. **Adding a collection becomes adding
catalogue data plus one editorial page, not authoring nine product pages.**
...
window of the year for the December test. Revised 2026-08-05: the mitigation is no longer a
temporary page on the old site. It is **one field on PKBA's existing inscription form**,
added now. Counting works through the rentrée regardless of when the new site lands, and
nothing about the December test depends on VertiFlow shipping anything.
...
Three targets, chosen 2026-08-05. The order path is already automated end to end —
catalogue sync → checkout → webhook → Printful draft → customer and owner emails — so
these are what genuinely remains manual.

**1. Printful confirmation latency.** The webhook creates drafts with `confirm: false` so
nothing reaches production unreviewed. The real problem isn't the two minutes of review,
it's that an order can sit unconfirmed for a week during the alternance.

Design: **auto-confirm after 24h unless vetoed**, with a notification the moment a draft
lands. The veto window survives, the customer never waits on a busy week, and the
irreversible step — a garment physically printed and shipped — keeps a human window in
...
**3. Visibility.** A read-only `/admin` route querying Stripe and Printful live, linking
out to Airtable. No database, no sync jobs, nothing to drift: the data stays in the systems
that already own it. Solves "five dashboards" without becoming a sixth.

**Explicitly not automated: publishing a new design.** Confirmed 2026-08-05 that this loop
is not a pain point. The migration therefore treats the dynamic product route as page
architecture, not as a time saving.

## The door

**Corrected 2026-08-05.** An earlier draft of this spec made the door the centre of the
project and gave it its own lead-capture form and Airtable base. That was wrong. The
December test is a *measurement*, not the product. The door is one route on the new site.

    Instagram post → profile → link in bio → /commencer
                   → PKBA's existing inscription flow → person attends

**`/commencer` answers six questions and links out.** What is this, can I do it, where,
when, what does it cost, will I be the worst one there. No form, no hero video, no origin
story. PKBA already runs registration into its own Airtable base — VertiFlow points at it
rather than duplicating it. That keeps the two entities separate by construction, gives
VertiFlow no personal data to hold, and removes a handoff that would otherwise have to be
owned by a named person.

**The first step should be free, not a licence.** Nobody pays for a licence to try a sport
they have never done. If PKBA does not currently run séances découverte, adding one is the
...
coin ?"* — a curated list of French parkour clubs, serving the followers who will never
reach La Teste and seeding the 2027 national play for free.

**Measurement** is one field on PKBA's existing inscription form — *"Comment tu nous as
connus ?"* — not a VertiFlow system. One field, no new base, no new GDPR surface.
...
| When | What |
|---|---|
Rewritten 2026-08-05. The door is no longer a workstream, so the only date-sensitive item
left is a single form field.

| When | What |
|---|---|
| **now** | Add *"Comment tu nous as connus ?"* to **PKBA's existing inscription form**. Five minutes. It must exist before the rentrée or December has no number, and nothing else in this plan depends on it. |
| Aug | Next.js foundation + commerce port · identity applied · brand-first pages |
| Sept | Collection artwork and Printful products · rentrée campaign on Instagram · beginner capture for `/commencer` |
| autumn | Live payments path — its own reviewed workstream, never compressed to hit a date |
| when ready | **Single launch** — new site + collection + live shop, cutover to `main` |
| after launch | `/admin` · Printful auto-confirm · content engine · journal queue |
| 31 Dec | Count |

**Plan sequence:**

1. Next.js foundation + commerce port (written: `2026-07-27-vertiflow-nextjs-foundation.md`)
2. Identity applied + brand-first pages, including `/commencer` and the journal
3. Collection, Printful products, reviewed catalogue invariant update
4. Live payments path
5. Launch and cutover
6. Automation: `/admin`, Printful auto-confirm, content engine

Done already: repo reconciled onto `origin/main`, 174-test baseline green, `BRAND.md` and
`brand.tokens.json` committed.
...
Re-ranked 2026-08-05 after the door was demoted from workstream to route.

1. **Scope drift on this spec itself.** Demonstrated, not hypothetical: an earlier draft
   turned a measurement into a subsystem with its own plan, form, Airtable base and GDPR
   surface, and two implementation sessions went into it before it was caught. The stated
   priority — refonte, then collection, then automation — is the check. Anything not
   serving one of those three is drift.
2. **The single launch pulls the payments path forward.** One unified launch means the
   ...
   serving the current site for as long as that takes.
3. **Auto-confirming Printful orders.** Money → an irreversibly printed and shipped garment
   with no human in the loop. The 24h veto window exists precisely so a catalogue error is
   catchable. Removing it to save two minutes an order at current volume is a bad trade.
4. **August's hours.** August belongs to klyx-estate by the 2026-07-14 decision; this work
   wants ~4h/week. If it takes more, it takes it from there. That should be a decision, not
   a discovery in September.
5. **Gold-plating the refonte.** Biggest item and the most enjoyable one. A working shop
   and a page that tells someone where to train matter more than any homepage animation.
6. **The measurement field never gets added.** One field on PKBA's inscription form, five
   minutes, and if it is not there before the rentrée then December has no number no matter
   how well everything else goes.
